...
from Telegram_bots.aiogram_6.state.register import RegisterState
...
async def start_register(message: Message, state: FSMContext, bot: Bot):
    ...
    if(users):
        await bot.send_message(message.from_user.id, f'{users[1]}, Вы уже зарегистрированы!')
    else:
        await bot.send_message(message.from_user.id, f'Давайте начнём регистрацию \nДля начала скажите как к Вам обращаться?')
